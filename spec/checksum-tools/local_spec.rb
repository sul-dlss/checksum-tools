...
describe Checksum::Tools::Local do
  ...
  describe "class methods" do
     ...
     it "should return false when registering an existing digest type" do
       (Checksum::Tools::Local.register_digest(:md5,'digest/md5') { Digest::MD5.new }).should be false
     end

     it "should return false when a digest class fails to load" do
       (Checksum::Tools::Local.register_digest(:sha999,'digest/sha999') { Digest::MD5.new }).should be false
     end
  ...
  describe "file operations" do
    ...
    it "should generate checksums for a tree of files" do
      ...
      File.exists?(File.join(@dir,'one/two/ignore.doc.digest')).should be false
    end
